"""Evaluation runner for Text-to-SQL systems."""
    ...
    for i, item in enumerate(dataset):
        ...
        try:
            ...
            results.append({
                ...
                "generated_sql": sql, "difficulty": difficulty,
                "correct": correct, "error": None,
                "latency_seconds": round(latency, 2),
                "input_tokens": sql_response.input_tokens,
                "output_tokens": sql_response.output_tokens,
            ...
            results.append({
                "question": question, "gold_sql": gold_sql,
                "generated_sql": "", "difficulty": difficulty,
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Text-to-SQL")
    ...
    print("Total:", metrics["total"], "Correct:", metrics["correct"],
          "Accuracy:", round(metrics["execution_accuracy"] * 100, 1), "%")
    print("Avg latency:", round(metrics["avg_latency_seconds"], 1), "s",
          "Avg tokens:", round(metrics["avg_tokens_per_query"]))
    print("Avg repairs:", round(metrics["avg_repairs"], 1),
          "Repair rate:", round(metrics["repair_rate"] * 100, 1), "%")
    print("=" * 60)
    for diff, stats in metrics["by_difficulty"].items():
        print(f"  {diff}: {stats["correct"]}/{stats["total"]} ({round(stats["accuracy"]*100)}%)")
